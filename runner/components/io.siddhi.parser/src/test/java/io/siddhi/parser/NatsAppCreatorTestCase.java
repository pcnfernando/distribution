...
//        siddhiManager.setExtension("nats-source", NatsSource.class);
     ...
     */
    @Test
    public void testFailOverTopologyCreation() {
    ...
    public void testSiddhiTopologyCreator() {
        String siddhiApp = "@App:name('Energy-Alert-App')\n"
                ...
                + " roomID string);\n"
                + "@sink(type = 'log', "
                + "@map(type = 'json', @payload('Device ID: {{deviceID}} of"
     ...
     */
    @Test
            //(dependsOnMethods = "testSiddhiTopologyCreator")
    public void testFilterQuery() {
        String siddhiApp = "@App:name('TestPlan2')"
                + "@source(type = 'http', topic = 'device-power', @map(type = 'json'))\n"
                + "define stream TempStream(deviceID long, roomNo int, temp double);\n"
                + "@info(name = 'query1') "
                + "@dist(parallel ='1', execGroup='001')\n "
                + "from TempStream\n"
                + "select *\n"
                + "insert into TempInternalStream;"
                + "@info(name='query2') "
                + "@dist(parallel='3',execGroup='002')\n"
     ...
     */
    @Test
    //(dependsOnMethods = "testPartitionWithWindow")
...
//                    + "</events>");
     ...
     */
    @Test
    //(dependsOnMethods = "testJoinWithPartition")
...
//                    + "</events>");
     ...
     */
    @Test
    //(dependsOnMethods = "testPartitionStrategy")
     ...
     */
    @Test
    //(dependsOnMethods = "testPartitionMultiSubscription")
     ...
     */
    @Test
    //(dependsOnMethods = "testPartitionWithMultiKey")
     ...
     */
    @Test
    //(dependsOnMethods = "testUserDefinedSink")
     ...
     */
    @Test
    //(dependsOnMethods = "testSinkStreamForSource")
     ...
     */
    @Test
    //(dependsOnMethods = "testUsergivenSourceNoGroup")
